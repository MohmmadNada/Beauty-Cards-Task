...
function scale_card(element_id, selected_color_names) {
    let card_element = document.getElementById(element_id)
    let color_div = card_element.querySelector("div")
    let button = document.querySelector(`[for='${element_id}']`)
    let selected_number = Number(document.getElementById("number-of-selected").textContent)
    let color_name = ntc.name(rgbToHex(color_div.style.backgroundColor))[1]
    ...
    let allColorsSelected = selected_color_names.join(" ")
